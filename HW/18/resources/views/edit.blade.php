...
@section('main')
    <div class="absolute py-10 bg-white opacity-80 rounded-md top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px]">
        <form class="w-full max-w-lg mx-auto" method="POST" action="{{ route('reserve.firstEdit', $reserve) }}">
            @csrf
            ...
            <div class="flex flex-wrap mb-6" id="submit">
                <button type="submit" id="ok"
                    class="hover:shadow-lg hover:shadow-zinc-900/70 text-white w-full bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Reserve</button>
            </div>
