...
abstract class Model 
{
    ...
    public function validate()
    {
        foreach ($this->rules() as $attribute => $ruls) {
            ...
            foreach ($ruls as $key => $rule) {
                ...
                if (self::RULE_WEEK === $ruleName) {
                    $container = array_filter($_POST, fn($val) => array_search($val, $_POST) !== '_method');
                    if (count($container) < WorkingTime::getDay()) {
                        $this->addError($attribute, self::RULE_WEEK);
                    }
                }
            }
        }
    ...
    public function errorMessage()
    {
        return [
            ...
            self::RULE_WEEK => 'The number of days selected must be more than four days'
        ];
    }
